...
class DATETIME(_DateTimeMixin, sqltypes.DateTime):
    ...
    The default string storage format is::
        ...
        "%(release_year)04d-%(month)02d-%(day)02d %(hour)02d:%(minute)02d:%(second)02d.%(microsecond)06d"
    ...
    ``storage_format`` and ``regexp`` parameters, such as::
        ...
        dt = DATETIME(storage_format="%(release_year)04d/%(month)02d/%(day)02d "
                                     "%(hour)02d:%(minute)02d:%(second)02d",
                      regexp=r"(\d+)/(\d+)/(\d+) (\d+)-(\d+)-(\d+)"
        )

    :param storage_format: format string which will be applied to the dict
     with keys release_year, month, day, hour, minute, second, and microsecond.
    ...
    _storage_format = (
        "%(release_year)04d-%(month)02d-%(day)02d "
...
class DATE(_DateTimeMixin, sqltypes.Date):
    ...
    ``storage_format`` and ``regexp`` parameters, such as::
        ...
        d = DATE(
                storage_format="%(month)02d/%(day)02d/%(release_year)04d",
                regexp=re.compile("(?P<month>\d+)/(?P<day>\d+)/(?P<release_year>\d+)")
            )

    :param storage_format: format string which will be applied to the
     dict with keys release_year, month, and day.
    ...
    _storage_format = "%(release_year)04d-%(month)02d-%(day)02d"
